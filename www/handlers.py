...
@get('/api/photo/{name}/delete')
async def api_photo_delete(name):
    name = os.path.abspath('.') + '/static/img/' + name
    try:
        os.remove(name)
    except:
        raise APIResourceNotFoundError('图片不存在')
    return name
...
def manage_photo():
    print('manage')
    rootdir = os.path.abspath('.') + '/static/img/'
    names = []
    ...
    for i in range(len(names)):
        names[i] = '/static/img/' + names[i]
    return {
        '__template__':'bs_manage_photos.html',
        'names':names
    }
