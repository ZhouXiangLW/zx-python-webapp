...
class RequestHandler(object):
	...
	async def __call__(self, request):
		kw = None
		if self._has_var_kw_arg or _has_named_kw_args or _required_kw_args:
...
def add_static(app):
	...
	logging.info('add static %s => %s', ('/static/'))
...
def add_route(app, fn):
	...
	if not asyncio.iscoroutinuefunction(fn) and not inspect.isgeneratorfunction(fn):
		fn = asyncio.coroutine(fn)
	logging.info('add route %s %s => %s(%s)' % (method, path, fn.__name__,','.join(inspect.signature(fn).parameters.keys())))
	app.router.add_route(method, path, RequestHandler(app, fn))
	
def add_routs(app, module_name):
	n = module_name.rfind('.')
	if n == (-1):
		mod = __import__(module_name, golobals(), locals())
	else:
		name = module_name[n+1:]
		mod = getattr(__import__(module_name[:n], golobals(), locals(), [name]),name)
	for attr in dir(mod):
